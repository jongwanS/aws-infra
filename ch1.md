...
- CloudFront(`CDN서비스`), Direct Connect, Route53(`DNS서비스`), AWS Shield(`DDos Protection`), AWS Global Accelerator가 `엣지`에서
  동작함.
...
- yaml 형식으로, OS, 인스턴스타입, 키 페어, 태그, 보안 그룹을 정의하여 배포 할 수 있으며, EC2 부팅시 자동으로 실행되는 명령어도 실행 시킬 수 있다.

### 2. CloudFormation 스택 생성
- 선언한 인스턴스의 EC2에, 웹서버 설치등이 자동적으로 실행된다.

### 3. CloudFormation 스택 삭제
- 단순히 CloudFormation을 삭제처리하면 EC2 인스턴스가 자동적으로 삭제가 된다.