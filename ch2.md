...
## 4. 실습, 퍼블리 서브넷 VPC 구성
- 목표 구성도  
![1](/images/ch2/public-subnet.png)
#### 단계
1. VPC 생성
2. 퍼블릭 서브넷 생성
3. 인터넷 게이트웨이(IGW) 생성, VPC 연결
4. 퍼블릭 라우팅 테이블 생성 및 서브넷 연결
5. 퍼블릭 라우팅 테이블 경로 추가


## 5. 실습, 프라이빗 서브넷 VPC 구성
- 프라이빗 서브넷은 `외부에서 내부로 인터넷 연결은 불가`, `내부에서 외부로는 인터넷 연결은 가능`
- 목표 구성도  

![1](/images/ch2/public-private-subnet.png)
#### 단계
1. 프라이빗 서브넷 생성
2. NAT 게이트웨이 생성
...
### 퍼블릭 서브넷과 프라이빗 서브넷의 통신흐름
#### - 퍼블릭 서브넷 통신 흐름
![1](/images/ch2/public-subnet-flow.png)

1. `퍼블릭 서브넷의 퍼블릭 EC2 인스턴스`가 외부 인터넷과 통신하기 위해 `데이터를 가상 라우터로 전달(퍼블릭 IP 사용)`
2. `가상 라우터는 퍼블릭 라우팅 테이블을 참조 인터넷 게이트웨이로 향하는 라우팅 경로를 확인`.
3. `가상 라우터는 인터넷 게이트웨이로 데이터를 전달`하고 인터넷 구간으로 넘어감.
4. 인터넷 구간을 통해 사용자에게 데이터 전달

#### - 프라이빗 서브넷 통신 흐름
![1](/images/ch2/private-subnet-flow.png)

1. `프라이빗 서브넷의 프라이빗 EC2 인스턴스`가 외부 인터넷 구간과 통신하기 위해 `데이터를 가상 라우터로 전달(프라이빗 IP 사용)`
2. `가상 라우터는 프라이빗 라우팅 테이블을 참조`, NAT 게이트웨이로 향하는 라우팅 경로를 확인
3. `가상 라우터는 NAT 게이트웨이로 데이터를 전달`하고, `NAT 게이트웨이에서 프라이빗 IP를 퍼블릭 IP로 전환`
4. `NAT 게이트웨이에서 인터넷 구간을 넘어가기 위해 인터넷 게이트웨이를 거쳐 사용자에게 전달`된다. 이때 사용자는 NAT 게이트웨이에서 `변환한 퍼블릭 IP로 전달`받는다.